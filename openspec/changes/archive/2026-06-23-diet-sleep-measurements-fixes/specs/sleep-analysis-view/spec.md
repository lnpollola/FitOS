...
The system SHALL provide a dedicated sleep analysis view accessible from the sidebar navigation, displaying sleep KPIs, phase breakdown charts, and a sleep duration timeline using data from the `activity_days` table populated via HealthSync Apple Health import.
...
The system SHALL render charts showing the distribution of sleep phases (deep, REM, light) over time, using a stacked bar or area chart with the organic color palette.
...
The system SHALL compute and display a sleep consistency metric using the coefficient of variation (CV = stddev / mean × 100) of sleep hours over the trailing 30 days.
...
The system SHALL compute a sleep trend by comparing the mean of the most recent 7 days against the mean of the preceding 7 days, and display a directional indicator.

#### Scenario: Improving trend
- **WHEN** the recent 7-day mean exceeds the prior 7-day mean by more than 0.5 hours
- **THEN** the system SHALL display "Mejorando" with an upward green arrow

#### Scenario: Worsening trend
- **WHEN** the recent 7-day mean is below the prior 7-day mean by more than 0.5 hours
- **THEN** the system SHALL display "Empeorando" with a downward red arrow

#### Scenario: Stable trend
- **WHEN** the difference between recent and prior 7-day means is ≤ 0.5 hours
...
- **THEN** the handler SHALL return totalAvg, deepAvg, remAvg, lightAvg, consistencyCV, consistencyLabel, and trend
