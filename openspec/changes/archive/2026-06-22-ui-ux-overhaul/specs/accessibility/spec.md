...
All animations and transitions in the app SHALL be disabled or reduced when the user has `prefers-reduced-motion: reduce` enabled. This extends the existing rule in `main.css:451` to cover new animations (skeleton pulse, card stagger, chart transitions).
