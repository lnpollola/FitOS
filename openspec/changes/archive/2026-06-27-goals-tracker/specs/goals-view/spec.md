...
1. **Active goals**: list of non-archived goals with progress rings, countdowns, and action buttons
...
Above the active goals, a prominent "Nuevo objetivo" button SHALL be present.

#### Scenario: Active goals list renders
- **WHEN** the goals view loads with active goals
- **THEN** each active goal SHALL render as a card with: progress ring (72 px), label, countdown, and action buttons (Editar, Archivar, Eliminar)
- **THEN** goals SHALL be ordered by `targetDate` ascending (soonest deadline first)
...
- **THEN** each completed goal SHALL show a full green ring and achievement badge

#### Scenario: Archived goals collapsible
- **WHEN** the user has archived goals
- **THEN** an expandable section "Objetivos archivados" SHALL appear at the bottom
- **THEN** clicking the toggle SHALL expand/collapse the list

#### Scenario: Empty state
- **WHEN** the user has no goals (empty array)
- **THEN** the view SHALL display an empty state: Lucide `target` icon (48 px), "Aún no tienes objetivos", and a "Crear objetivo" button
