...
- **THEN** on success: `handler(args)` SHALL return `{ ok: true, data: <result> }`
- **THEN** on error: `handler(args)` SHALL return `{ ok: false, error: <message> }`
...
- **THEN** unused parameters SHALL NOT be prefixed with underscore (for consistency)

### Requirement: Measurements handler bug fixed

`src/main/handlers/measurements-handlers.js` SHALL declare `notifyDomain` as a named parameter so the references at lines 26 and 44 do not throw ReferenceError.
