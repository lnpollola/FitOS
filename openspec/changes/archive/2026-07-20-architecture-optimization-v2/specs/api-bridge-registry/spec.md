...
Every channel invocable in Electron mode SHALL be invocable in Web mode (and vice versa), except explicitly excluded platform-only operations (e.g., HealthSync CLI install). The manifest SHALL mark any such exclusion explicitly with a `platforms` field.
