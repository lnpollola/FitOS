...
Provide a reusable SVG sparkline renderer (`src/renderer/utils/sparkline.js`) that any view can call to draw an inline smooth microchart of a numeric time series. Sparklines are NOT Chart.js instances — they are pure SVG strings with no JS charting dependency, no animations, and a small CSS footprint. They give metric cards a "live series" feel without pulling in a chart library per card.
